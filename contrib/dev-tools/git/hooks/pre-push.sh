...
print_usage() {
    cat >&2 <<'EOF'
Usage: ./contrib/dev-tools/git/hooks/pre-push.sh [--format=<text|json>] [--verbosity=<concise|verbose>] [--verbose]
...
Environment:
  TORRUST_GIT_HOOKS_LOG_DIR     Shared directory for per-step log files (used by all git hooks). Default: /tmp
EOF
}
...
echo
echo "=========================================="
echo "FAILED: Pre-push checks failed!"
