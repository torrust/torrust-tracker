...
`namespace` identifies the declaration format rather than the repository that carries it, so every Torrust repository uses the same value, `com.torrust.repository.symlinks`. A declaration is bound to its repository by where it is read from, that repository's own merged tree, so no string inside the file adds a guarantee on top of that. `version` is the version of the declaration format, not of the repository. Both fields are required, and a declaration is read only when it carries exactly the values shown above.
...
- The declaration must be strict JSON: `NaN`, `Infinity` and `-Infinity`, which the tool's JSON reader accepts by default although JSON defines none of them, are rejected wherever in the document they appear, so a file that no conforming reader would accept exempts nothing and takes the same path as any other file that is not valid JSON.
- The header is checked before any entry is read: `namespace` must be exactly `com.torrust.repository.symlinks` and `version` exactly `[1, 0, 0]`, each matched as the JSON value it is, so `1.0` or `true` where an integer is documented is not that value. A document that omits either field, or carries another value for it, belongs to another format or to a revision of this one whose rules the tool has not been taught: it exempts nothing, the report names the field that disagreed, and every link refuses. A later revision of the format has to teach the tool its version rather than be read under the rules of the revision the tool does know.
