...
impl Response {
    ...
    pub fn parse_bytes(mut bytes: &[u8], ipv4: bool) -> Result<Self, io::Error> {
        ...
        match action.get() {
            ...
            // Announce
            1 if ipv4 => {
                let fixed = AnnounceResponseFixedData::read_from_prefix(bytes).ok_or_else(invalid_data)?;

                let peers = if let Some(bytes) = bytes.get(size_of::<AnnounceResponseFixedData>()..) {
                    Vec::from(ResponsePeer::<Ipv4AddrBytes>::slice_from(bytes).ok_or_else(invalid_data)?)
                } else {
                    Vec::new()
                };
            ...
            1 if !ipv4 => {
                ...
                let peers = if let Some(bytes) = bytes.get(size_of::<AnnounceResponseFixedData>()..) {
                    Vec::from(ResponsePeer::<Ipv6AddrBytes>::slice_from(bytes).ok_or_else(invalid_data)?)
                } else {
                    Vec::new()
                };
            ...
            2 => {
                ...
                let torrent_stats = Vec::from(TorrentScrapeStatistics::slice_from(bytes).ok_or_else(invalid_data)?);
