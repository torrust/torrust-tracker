...
impl Request {
    ...
    pub fn parse_bytes(bytes: &[u8], max_scrape_torrents: u8) -> Result<Self, RequestParseError> {
        ...
        match action.get() {
            ...
            1 => {
                let request =
                    AnnounceRequest::read_from_prefix(bytes).ok_or_else(|| RequestParseError::unsendable_text("invalid data"))?;
            ...
            2 => {
                ...
                let info_hashes = FromBytes::slice_from(remaining_bytes)
                    .ok_or_else(|| RequestParseError::sendable_text("Invalid info hash list", connection_id, transaction_id))?;
