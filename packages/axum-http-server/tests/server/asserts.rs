...
pub async fn assert_announce_response(response: Response, expected_announce_response: &DeserializedNormal) {
    ...
    let announce_response: DeserializedNormal = serde_bencode::from_bytes(&body)
...
pub async fn assert_compact_announce_response(response: Response, expected_response: &DeserializedCompactParsed) {
    assert_eq!(response.status(), 200);
    ...
    let actual_response = DeserializedCompactParsed::from(compact_announce);
