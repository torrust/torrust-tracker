...
pub fn run_persistent_completed_statistics_event_listener(
    ...
    tokio::spawn(async move {
        dispatch_persistent_completed_statistics_events(receiver, cancellation_token, db_downloads_metric_repository).await;
...
async fn dispatch_persistent_completed_statistics_events(
    ...
    loop {
        tokio::select! {
            ...
            result = receiver.recv() => {
                match result {
                    Ok(event) => handle_persistent_completed_statistics_event(event, &db_downloads_metric_repository).await,
                    Err(e) => {
                        match e {
                            RecvError::Closed => {
                                tracing::info!(target: TRACKER_CORE_LOG_TARGET, "Tracker core event receiver closed");
