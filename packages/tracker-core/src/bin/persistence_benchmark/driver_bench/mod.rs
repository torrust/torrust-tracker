...
pub async fn run(driver: Driver, db_version: &str, ops: OpsCount) -> Result<Vec<RawOperationSamples>> {
    ...
    let db = active_database.database.as_deref().unwrap().as_ref();
    database::reset_database(db).await?;

    let ops = ops.get();

    let mut operations_samples = Vec::new();
    operations::benchmark_torrent_operations(db, ops, &mut operations_samples)?;
    operations::benchmark_whitelist_operations(db, ops, &mut operations_samples)?;
    operations::benchmark_key_operations(db, ops, &mut operations_samples)?;
