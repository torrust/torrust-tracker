...
pub(super) fn benchmark_whitelist_operations(
    ...
    operations.push(measure_operation("add_info_hash_to_whitelist", ops, |index| {
        let info_hash = info_hash_from_index(30_000 + index)?;
    ...
    operations.push(measure_operation("get_info_hash_from_whitelist", ops, |_| {
    ...
    operations.push(measure_operation("load_whitelist", ops, |_| {
        let whitelist = database.load_whitelist().context("load_whitelist failed")?;
        drop(whitelist);
        Ok(())
    })?);

    operations.push(measure_operation("remove_info_hash_from_whitelist", ops, |index| {
