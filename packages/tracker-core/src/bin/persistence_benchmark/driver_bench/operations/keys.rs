...
pub(super) fn benchmark_key_operations(
    ...
    operations.push(measure_operation("add_key_to_keys", ops, |_| {
    ...
    operations.push(measure_operation("get_key_from_keys", ops, |_| {
    ...
    operations.push(measure_operation("load_keys", ops, |_| {
        let keys = database.load_keys().context("load_keys failed")?;
        drop(keys);
        Ok(())
    })?);

    operations.push(measure_operation("remove_key_from_keys", ops, |_| {
        let peer_key = authentication::key::generate_key(None);
        let _added_rows = database
