...
One detail of the repair is load-bearing rather than cosmetic. `.yamllint-ci.yml` sets `line-length: max: 200`, and `linter all` runs yamllint over the workflow files. Line 185 is currently 184 characters; adding `-p torrust-tracker-e2e-tools ` costs 26 more and would take it to 210, over the limit. The other three lines are 153, 151, and 156 characters and land at 179, 177, and 182. The first step must therefore be wrapped in a `>-` block scalar, which is exactly the shape `container.yaml` already uses for the same step and for the same reason.
