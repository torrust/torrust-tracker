...
related-pr: null
last-updated-utc: 2026-09-08 19:24
semantic-links:
  skill-links:
    - create-issue
  related-artifacts:
...
- The declaration is read from the final merged tree only, out of the local merge commit the tool has just created, with `git show HEAD:<path>` or the equivalent `git cat-file` against that commit, where `<path>` is the value of `--symlinks` and `.symlinks.json` when the argument is omitted. It is never read from the working directory, the index, the base branch, or an intermediate commit in the range, and never from any path outside the repository.
...
- [ ] AC2: The tool reads the declaration from the final merged tree, out of the local merge commit it has just created, at the tree path `--symlinks` names, and from no other source. A declaration present only in the working directory, only on the base branch, or only in an intermediate commit without reaching the merged result exempts nothing.
