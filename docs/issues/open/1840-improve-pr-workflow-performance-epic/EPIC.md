...
| 10    | #1869 (superseded by row 7) — Split cook layer investigation (superseded, see below)         | `docs/issues/drafts/1840-workflow-performance-split-external-dep-cache-layer/ISSUE.md`          | SUPERSEDED | Resolved by `--external-only` flag in `torrust-cargo-chef` fork during #1869 investigation. The three-layer cook pattern (third-party-only cook → full cook → build) is now tracked directly under #1869. Draft kept as investigation archive.                                                                                                                                              |
