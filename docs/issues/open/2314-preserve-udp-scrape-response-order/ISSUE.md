...
| B3 | DONE | Read the current UDP scrape tests and load `write-unit-test` | The `scrape_request` collaboration tests already exercise `handle_scrape` with public-tracker services. Extend only their request and peer-seeding mechanics; the tests retain their ordered hashes, expected typed vector, production Act, and assertion. Design recorded in `manual-verification-evidence.md`. |
| B4 | DONE | Add the deterministic duplicate-entry red regression | `it_should_return_an_entry_for_each_duplicate_requested_info_hash` failed as intended: the response contained one typed entry for `[A, A]`. Command and output summary: `manual-verification-evidence.md`. |
| B5 | DONE | Add the high-confidence order red regression | `it_should_preserve_the_order_of_eight_requested_info_hashes` failed as intended: returned counts did not match the requested positional sequence. `N = 8`, accidental-pass bound $1/40320$. Command and output summary: `manual-verification-evidence.md`. |
...
- 2026-09-23 - GitHub Copilot - Started implementation on
  `2314-preserve-udp-scrape-response-order`. Completed B3: loaded `write-unit-test`, inventoried
  the existing `udp-server` scrape-handler collaboration tests, and recorded the prose-first
  designs for the deterministic duplicate and high-confidence order regressions in
  `manual-verification-evidence.md`.
- 2026-09-23 - GitHub Copilot - Completed B4-B6. Both new handler collaboration tests failed
  against the unfixed implementation: `[A, A]` returned one entry, and an eight-hash non-sorted
