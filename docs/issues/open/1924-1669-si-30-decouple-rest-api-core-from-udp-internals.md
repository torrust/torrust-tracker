...
`udp-core` as runtime dependencies. After the decoupling, they can be demoted to
...
- `UdpTrackerCoreContainer` already holds `Arc<UdpTracker>`, so `container.rs` reads
  `udp_tracker_config.max_connection_id_errors_per_ip` instead of the constant.
...
This eliminates the dependency edge entirely and is more aligned with the project's
observability and testability principles (configuration-driven, not hardcoded).
...
- [ ] `rest-api-core/Cargo.toml`: `udp-server` + `udp-core` are dev-deps only
...
- [ ] `axum-rest-api-server/Cargo.toml`: `udp-server` + `udp-core` are dev-deps only
