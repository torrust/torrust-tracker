...
| T11 | TODO   | Analyse test binary landscape and document optimisation opportunities        | Documented in T11 section above. Three concrete opportunities identified: exclude console client, move e2e runners to own package, move testcontainers to dev-deps.                     |
| T12 | TODO   | Exclude `torrust-tracker-client` from `cargo nextest archive`                | Add `--exclude torrust-tracker-client` to all three archive calls (debug cook, release cook, build archive) in Containerfile. Removes 4 bins + 2 integration test harnesses.            |
| T13 | TODO   | Move `e2e_tests_runner` and `qbittorrent_e2e_runner` to a separate package   | Create `packages/e2e-tools/` (or similar) package. Move the two bins from root `src/bin/`. Add the new package to `--exclude` list in Containerfile. Update recipe/stub stanzas.        |
| T14 | TODO   | Move `testcontainers` to `[dev-dependencies]` in `tracker-core`              | Verify all usages are inside `#[cfg(test)]` or `persistence_benchmark_runner`. Move dep, gate the bin if needed. Reduces production binary size and link overhead across the workspace. |
...
- [ ] AC12: Test binary landscape is documented with a count and categorisation of all targets compiled by `cargo nextest archive --tests` after existing exclusions.
- [ ] AC13: `torrust-tracker-client` console package is excluded from all three `cargo nextest archive` calls in the Containerfile.
- [ ] AC14: `e2e_tests_runner` and `qbittorrent_e2e_runner` binaries are moved to a separate package and excluded from the archive.
- [ ] AC15: `testcontainers` is declared as `[dev-dependencies]` in `packages/tracker-core/Cargo.toml` and no longer linked into production binaries.
