...
| T3  | TODO   | Implement package summary and discovery        | Use Cargo metadata plus `git diff --name-only "$base_sha" "$head_sha"` to identify directly changed workspace packages. Produce exact source-only per-package covered/instrumented-line summaries from existing workspace reports. Define root, new, removed, renamed, and workspace-config behavior.                 |
