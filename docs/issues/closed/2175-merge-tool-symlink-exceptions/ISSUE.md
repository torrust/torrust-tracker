...
epic: null
github-issue: 2175
spec-path: docs/issues/closed/2175-merge-tool-symlink-exceptions/ISSUE.md
...
semantic-links:
  ...
  related-artifacts:
    ...
    - docs/issues/closed/2175-merge-tool-symlink-exceptions/verification.md
...
- The document is parsed as strict JSON before any rule of this format is applied: the three constants JSON does not define, `NaN`, `Infinity` and `-Infinity`, are rejected wherever in the document they appear, because the reader the tool is built on accepts all three by default and a document carrying one would otherwise be read here while every conforming reader refuses it. Such a document is a declaration that cannot be read: it exempts nothing, the tool names the constant it found, and every link refuses exactly as an absent declaration does.
- The header is validated before any entry is read: `namespace` must be exactly `com.torrust.repository.symlinks` and `version` exactly `[1, 0, 0]`, each matched as the JSON value it is rather than by a comparison that treats `1.0` or `true` as the documented integer. A document that omits either field, or carries another value for it, is a document of another format or of a revision this tool has not been taught, and it exempts nothing: the tool names the field that disagreed and refuses every link exactly as an absent declaration does.
...
- 2026-09-09 14:00 UTC - Implementer - Answered the PR #2184 review of the implementation: the declared target is now matched against the bytes the tree carries rather than a decoded rendering of them, so a target that is not valid UTF-8 has no declaration it can equal and a path outside ASCII matches the entry that names it; every path, target, and reason the report prints is escaped, so tree content cannot forge a line of it; and the range walk reads each distinct link content once instead of once per commit that carries it - https://github.com/torrust/torrust-tracker/pull/2184
- 2026-09-09 14:20 UTC - Implementer - Stated the declaration path as a caller's convention wherever it was described as a property of the mechanism: the objective no longer names a root file, and the vendoring README and the merge skill say the tool reads whatever tree path `--symlinks` names and knows no name of its own. AD2 already decided this; what changed is that the documents now say it where a reader meets the file - https://github.com/torrust/torrust-tracker/pull/2184
- 2026-09-09 16:30 UTC - Implementer - Reversed the reading recorded at 10:10 UTC, that the header was documented but not enforced: a check that decides what a merge admits cannot read a document that has not said which format it belongs to, so the tool now validates `namespace` and `version` before it reads a single entry, and a document that omits either or carries another value exempts nothing and is reported by the field that disagreed. The comparison is exact and type-strict, since JSON's `true` and `1.0` are not the documented integer although Python compares them equal, and an unsupported version fails closed rather than being read under the rules of the version this tool does know, so a later revision of the format has to teach the tool its version - https://github.com/torrust/torrust-tracker/pull/2184
- 2026-09-10 06:30 UTC - Implementer - Closed the gap between the rule that a declaration which is not valid JSON exempts nothing and what the reader actually accepted: Python's JSON reader admits `NaN`, `Infinity` and `-Infinity`, none of which JSON defines, so a document carrying one was read here although every conforming reader refuses it, and the fail-closed rule never reached it. The reader now rejects all three wherever they appear, through the same failure the rule already covers, so the document is one that cannot be read, the report names the constant found, and every link refuses as with no declaration at all. The two remaining JSON reads in the tool parse GitHub API responses, which are a different trust class - they cannot exempt a link and are already discarded on any exception - so they are unchanged - https://github.com/torrust/torrust-tracker/pull/2184
