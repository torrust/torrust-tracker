...
| Task Reviewer | Committer          | Task Reviewer reports `REVIEW PASSED` for an implementation commit.                 | Verified scope and acceptance-criteria evidence, including an issue-local report when the reviewer received a folder-style specification. | Committer returns stale-spec, policy, implementation, or test blockers to the code-focused owner. |
...
| PR Reviewer                 | A review request for an existing PR and its actual diff/check context, whether open or merged. | Open PR: findings and `APPROVE`, `REQUEST_CHANGES`, or `COMMENT`; merged PR: `POST_MERGE_FINDINGS` and a handoff to `process-pr-review`. | Review is read-only. The profile does not own remediation, thread publication, or required re-review after fixes; a merged-PR review does not authorize a follow-up branch. |
| Copilot Suggestions Handler | Copilot review feedback on an open or merged PR.                          | Open PR: per-thread decision, reply, resolution, and unified audit; merged PR: read-only triage followed by explicit maintainer approval, with approved follow-up evidence returned to the audit. | It replies before resolving open-PR threads and fetches threads again after each push. For merged PRs it must not branch, edit, commit, push, reply, or resolve before approval; approved fixes branch from the current target branch. |
...
| Pull-request review audit         | PR author.                                                          | `docs/pr-reviews/pr-<number>-review/PR-REVIEW.md`.                                              | Records all review findings, dispositions, replies, and resolution state.                              |
