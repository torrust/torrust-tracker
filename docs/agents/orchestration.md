...
| PR Reviewer                 | A review request for an existing PR, whether open or merged.             | Open PR: findings and `APPROVE`, `REQUEST_CHANGES`, or `COMMENT`; merged PR: `POST_MERGE_FINDINGS` and a handoff to `process-pr-review`. | The profile does not own remediation. A merged-PR review is read-only and does not authorize a follow-up branch.                                                       |
