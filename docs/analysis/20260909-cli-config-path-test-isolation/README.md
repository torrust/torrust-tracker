...
guard for all seven current fixture consumers.
...
| Seven current `TrackerApplicationFixture` consumers                        | In-process integration executables        | Indirectly use the shared fixture above                                                                                      | No                                                       | Yes, through shared fixture               | No per-suite migration should be necessary. Consumers: two UDP banning suites, the UDP disabled metrics suite, fixed-port metrics, port-zero metrics, two UDP-error metrics suites, and `scaffold`.                                          |
...
`tests/common/workspace.rs`; `scaffold` demonstrates the same API for future
suites.
