...
impl Service {
    ...
    pub async fn run_checks(self) -> Result<Vec<CheckResult>, JoinError> {
        ...
        checks
            .spawn(udp::run(self.config.udp_trackers.clone(), timeout).map(|mut f| f.drain(..).map(CheckResult::Udp).collect()));
        checks.spawn(
            http::run(self.config.http_trackers.clone(), timeout).map(|mut f| f.drain(..).map(CheckResult::Http).collect()),
        );
        checks.spawn(
            health::run(self.config.health_checks.clone(), timeout).map(|mut f| f.drain(..).map(CheckResult::Health).collect()),
        );
