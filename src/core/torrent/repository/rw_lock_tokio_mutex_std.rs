...
use crate::core::torrent::{entry, SwarmMetadata, TorrentsRwLockTokioMutexStd};
...
impl UpdateTorrentAsync for TorrentsRwLockTokioMutexStd {
...
impl UpdateTorrentAsync for Arc<TorrentsRwLockTokioMutexStd> {
    async fn update_torrent_with_peer_and_get_stats(&self, info_hash: &InfoHash, peer: &peer::Peer) -> (bool, SwarmMetadata) {
        self.as_ref().update_torrent_with_peer_and_get_stats(info_hash, peer).await
    }
}

impl Repository<entry::MutexStd> for TorrentsRwLockTokioMutexStd {
